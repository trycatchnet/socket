...
int main(int argc, char** argv) {
    ...
    request_hdr.msg_type = htons(0x0001); // Binding request
    request_hdr.msg_length = htons(0);    // not Attribute for now
    request_hdr.magic_cookie = htonl(0x2112A442);
    ...
    if (response_hdr.msg_type == htons(0x0101))
        printf("(Binding Success)\n");
    else if (response_hdr.msg_type == htons(0x0111)) {
    ...
    struct in_addr mapped_addr_struct = { .s_addr = mapped_ip };
    
    printf("MAPPED (Public) Address\nIP Address: %s", inet_ntoa(mapped_addr_struct));
    printf("Port: %d\n\n", mapped_port);
