...
int p2p_send_chat(int sock_fd, const struct sockaddr_in *dst, uint32_t seq, const char *text, size_t text_len) {
    ...
    /* Wire layout: [ 17 byte header ][ raw chat text bytes ] */
    uint8_t buf[MAX_BUFFER_SIZE];

    int hlen = p2p_pack(&pkt, buf, sizeof(buf));
    if (hlen < 0) return -1;

    /* A single UDP datagram can't carry more than our buffer anyway,
     * so truncate instead of failing/splitting the message. */
    if (text_len > (size_t)(MAX_BUFFER_SIZE - hlen))
        text_len = (size_t)(MAX_BUFFER_SIZE - hlen);

    memcpy(buf + hlen, text, text_len);

    return (int)sendto(sock_fd, buf, (size_t)hlen + text_len, 0, (const struct sockaddr *)dst, sizeof(*dst));
}
...
void chat_loop(int sock_fd, const struct sockaddr_in *peer_addr) {
    ...
    printf("\n[CHAT] Connected with %s:%d - Type a message and press ENTER to send.\n"
           "       Ping keeps running in the background to watch the connection.\n"
           "       Type /quit (or CTRL+C) to exit.\n\n", peer_ip_str, peer_port);

    /* Sequence number for the next ping */
    uint32_t seq = 0;

    /* Sequence number for the next chat message, counted separately from ping seq */
    uint32_t chat_seq = 0;
    ...
    /* pfds[0] : the UDP socket, same as before (ping/pong/bye/chat packets)
     * pfds[1] : stdin (keyboard), new - lets us send chat without blocking pings */
    ...
    while (!g_stop) {
        ...
        int pr = poll(pfds, 2, POLL_SLICE_MS);
        ...
        if (pr > 0 && (pfds[0].revents & POLLIN)) {
            ...
            if (n > 0 && addr_equal(&from, peer_addr)) {
                ...
                if (p2p_unpack(buf, (size_t)n, &pkt) == 0) {
                    ...
                    /* Peer sent us a chat message: everything after the fixed
                     * header is the raw text. Bound and null-terminate it
                     * before printing, since it isn't null-terminated on the wire. */
                    else if (pkt.type == MSG_CHAT) {
                        size_t text_len = (size_t)n - P2P_PACKET_SIZE;

                        char text[MAX_BUFFER_SIZE - P2P_PACKET_SIZE + 1];

                        if (text_len > sizeof(text) - 1) text_len = sizeof(text) - 1;

                        memcpy(text, buf + P2P_PACKET_SIZE, text_len);
                        text[text_len] = '\0';
        ...
        /* Stdin (keyboard) has a line ready: read it and send it as chat.
         * Runs independently from the ping timers above, so typing a
         * message never delays the background ping/pong checks. */
        if (pr > 0 && (pfds[1].revents & POLLIN)) {
            char line[1024];

            if (!fgets(line, sizeof(line), stdin)) {
                /* stdin closed (EOF / CTRL+D): treat it like /quit */
                printf("\n[INFO] stdin closed, exiting chat.\n");
                g_stop = 1;
            } else {
                /* strip the trailing newline fgets keeps, if any */
                size_t len = strlen(line);
                if (len > 0 && line[len - 1] == '\n') line[--len] = '\0';

                if (len == 0) {
                    /* empty line, nothing to send */
                } else if (strcmp(line, "/quit") == 0) {
                    printf("[INFO] Exiting chat.\n");
                    g_stop = 1;
                } else {
                    chat_seq++;
                    p2p_send_chat(sock_fd, peer_addr, chat_seq, line, len);
                    printf("[You] %s\n", line);
                }
            }
        }
